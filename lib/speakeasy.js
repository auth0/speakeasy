...
speakeasy.generate_key = function(options) {
  ...
  if (google_auth_qr) {
    ...
    name = name.replace(/ /g,'');
    SecretKey.google_auth_qr = 'https://chart.googleapis.com/chart?chs=166x166&chld=L|0&cht=qr&' +
                               'chl=otpauth://totp/' + encodeURIComponent(name) +
                                   '%3Fsecret=' + encodeURIComponent(SecretKey.base32) +
                                   '%26issuer=' + encodeURIComponent(issuer);
